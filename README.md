...
2) Verifica si existe, imagenes_procesadas, si es asi, veo el contenido de la carpeta imagenes_procesadas con el comando 'ls',luego aplico una tuberia para poder obtener la informacion necesaria, de la siguiente manera:
 aplico el comando 'awk -F 'recortada' ' {print $1}. Lo que efectua este comando es tomar el contenido del nombre con la terminacion '_recortada.jpg', utilizando el separador '_recortada' y muestra solo la primera parte de cada linea . Por ej, Luciano_Domingo_recortada.jpg , obtendre Luciano_Domingo. Genera el archivo lista_de_nombres_validos.txt y emite el mensaje de que se genero el archivo .
